    ...
    callback: async (bot: Client, interaction: ChatInputCommandInteraction) => {
        ...
        collector.on('collect', async (interactionButton: ButtonInteraction) => {
          ...
          /*
                ...
                //
                let promises: any[] = [];
                boosters.forEach(boosterId => promises.push(Bank.findOne({
                    userId: boosterId,
                    guildId: interactionButton.guildId,
                })));


                const banks = await Promise.all(promises);
                const embedDataPayout: EmbedData = buildPayout(boostId, boosters, banks, amounts);

                let rows: any[] = [];
                let n: number = 1;
                let row: any = new ActionRowBuilder();
                for (const _ of boosters) {
                    row.addComponents(new ButtonBuilder()
                        .setLabel(`\`${String(n).padStart(2, '0')}\``)
                        .setEmoji('✉️')
                        .setStyle(ButtonStyle.Danger)
                        .setCustomId(`pcr.payout.${boostId}.${n}`));
                ...
                await interactionButton.editReply('useless')

                interactionButton.channel!.send({
                ...
                /*
                const replyPayouts: any = await interactionButton.editReply({
                    content: '# Etat des transferts',
                    embeds: [embedPayouts],
                    components: rows
                });
                                    ...
                                    // collect button clicks & listen for them
                                    const coll: InteractionCollector<any> = replyPayouts.createMessageComponentCollector({
                                        componentType: ComponentType.Button,
                                        filter,
                                    });


                                    console.log(replyPayouts.id)

                                    coll.on('collect', async (ib: ButtonInteraction) => {
                                        await ib.reply('hey!');
                                        //const bb = await ib.deferReply();
                                        //console.log(bb)
                                        return;
                                    });*/
